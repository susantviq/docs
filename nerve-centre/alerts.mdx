...
A real-time (`RT`) Hero alert on Google Ads. Fires when today's ROAS drops more than 25 percent versus the average ROAS on the same day of week over the prior seven days. The card is suppressed before 14:00 local time to avoid false positives from incomplete morning data (Google Ads conversion data has a 1-4 hour ingestion lag).
