...
| [google_ads.cost](/nerve-centre/google_ads/cost) per-Item | Indirect, but critical | An eroding-margin Item still receiving high ad spend is the killer cross-channel finding. Intacct says "this product is unprofitable", Google Ads says "we are spending money to acquire customers for this product". The intersection is "we are paying to lose money", which the [Dead Stock with Active Ad Spend](/nerve-centre/kpi-cards/sage/dead-stock-with-active-ad-spend) cross-channel card surfaces explicitly. |
...
Not by default. The card uses Chart-mapped COGS only, which excludes Amazon / eBay marketplace fees (usually OpEx), Stripe / PayPal processing fees (usually OpEx), and ad spend (always OpEx). For a "true contribution margin" view, pair with the [Cross-Channel Contribution Margin](/nerve-centre/sage/sage_xc_contribution_margin) card, which subtracts these per-Item. The headline difference is significant: an Item showing 35% gross margin on this card may be at 22% contribution margin once ad spend, marketplace fees, and processing are loaded in. The boardroom number stays as gross; the operating decision uses contribution.
{/* /HUMAN_AUTHORED:faqs */}
