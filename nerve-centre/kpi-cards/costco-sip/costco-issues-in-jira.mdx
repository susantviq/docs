...
| **Cross-platform only flag** | `only_when: has_jira_sibling`. Card hides if no Jira connector is integrated. |
...
This card is the supplier's internal cross-platform join; Costco's supplier portal has no view of internal Jira tickets. There is nothing to reconcile against on the Costco side.
...
Cross-platform Jira-tagged escalation queues exist with conceptually similar definitions across other wholesale programs.
