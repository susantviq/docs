...
Yes. The sub-signal weights and the 70 trigger are configurable per profile in the Alert Rules tab. Teams with strict latency SLAs often raise the latency weight; teams running near disk capacity often raise the capacity weight. Tune to your own baseline rather than relying on the generic default.
