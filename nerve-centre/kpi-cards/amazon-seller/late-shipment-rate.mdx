...
Yes. The `>3.5% (warn) / >4% (breach)` defaults are configurable per profile in the Alert Rules tab. Amazon enforces its own ceiling regardless, so keep the warn line below Amazon's target to give yourself reaction time.
{/* /HUMAN_AUTHORED:faqs */}
