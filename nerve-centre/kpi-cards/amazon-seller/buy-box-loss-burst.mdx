...
1. **Four top earners lost the Buy Box in one morning.** That is past the `>3` threshold, so the Key Metrics card fires. The clustering, two of them to the same 6% competitor price move, is the tell that this is systemic, not random.
