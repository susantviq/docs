...
| **What counts as a "checkout drop"** | A 5-minute checkout-completion rate that falls below the trailing baseline by more than the alert threshold for the linked storefront. The drop must be measured, not inferred, so the card needs a connected storefront to populate the cross-platform half. |
...
**The checkout side.** The linked storefront connector reports checkout-completion events per minute. Vortex IQ computes a 5-minute completion rate and compares it to the trailing baseline for that storefront and time-of-week. A fall beyond the alert threshold marks the window as a "checkout drop".
...
The storefront connector reports checkout completion has fallen from a trailing baseline of 41 completions per 5 minutes to 18, a 56% drop, which crosses the alert threshold and marks the window 20:00 to 20:05 as a checkout drop. The card lists the slow queries that fired in the same bucket:
...
| --- | --- | --- |
| [`shopify.checkout-completion-rate`](/nerve-centre/kpi-cards/shopify/total-revenue) / BigCommerce / Adobe Commerce equivalents | The storefront-side drop that gates this card should match the storefront connector's own checkout metric for the same window. | A drop here but not on the storefront card equals an alert-threshold mismatch; align the thresholds. |
