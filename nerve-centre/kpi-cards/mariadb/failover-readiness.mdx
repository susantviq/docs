...
| **Alert Rules** | Alert Rules card: the maximum acceptable replica lag and the readiness criteria are tunable per profile, because the lag a business can tolerate on failover differs by workload. |
