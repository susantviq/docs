...
Known internal jobs (nightly reindex, analytics export, ETL) produce exactly the no-order spike fingerprint. Exclude them by source IP or user in the Alert Rules tab, or schedule them in a window the card knows to discount. Do not widen the threshold globally, as that would also hide real scrapers.
