...
Yes, the day threshold (default 7) is configurable per profile in the Alert Rules tab. Set it just above your normal settlement cycle so the alert fires only when a payout is genuinely overdue rather than on routine timing.
{/* /HUMAN_AUTHORED:faqs */}
