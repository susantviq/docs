...
| **Alert trigger** | `>0 drift between doc count and ecom product count`. Because a healthy sync should be exact, any non-zero gap raises an alert rather than waiting for a percentage threshold. |
...
| [MongoDB Health Score](/nerve-centre/kpi-cards/mongodb/mongodb-health-score) | Persistent drift should be visible as a data-integrity concern. | If the score ignores drift, check whether data-integrity signals are weighted in the alert profile. |
{/* /HUMAN_AUTHORED:reconcile */}
