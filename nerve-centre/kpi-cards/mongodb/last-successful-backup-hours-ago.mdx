...
| **Alert trigger** | `>72h`. Any deployment whose newest successful backup is older than 72 hours raises an alert. Most teams tighten this well below 72h once a daily or continuous schedule is in place. |
...
| [MongoDB Health Score](/nerve-centre/kpi-cards/mongodb/mongodb-health-score) | A red backup age should drag the health score down. | If health score stays green with a stale backup, check the score's backup weighting in your alert profile. |
{/* /HUMAN_AUTHORED:reconcile */}
...
72h is a deliberately conservative default so it does not cry wolf on weekly or every-other-day schedules. It is the line past which most teams have no usable recovery point. If your RPO is tighter, lower the alert threshold to one or two backup intervals so you are warned after a single missed run, not three.
