...
If your fill rate is fast or your provisioning lead time is long, yes. The 90% default is a backstop. Many teams set a profile-level threshold at 80% in the Alert Rules tab so the warning arrives with more runway. The right line depends on how quickly you can add storage: an Atlas cluster that resizes online in minutes can tolerate a higher threshold than a self-hosted volume that needs a maintenance window.
