...
| **Alert trigger** | `>1%`. An error rate above 1% over the 5-minute window raises an alert. For most order-backing workloads even 1% is a lot of failed user actions, so many teams tighten this. |
...
| [MongoDB Health Score](/nerve-centre/kpi-cards/mongodb/mongodb-health-score) | A red error rate should pull the health score down. | If the score stays green during an error spike, check its error weighting in the alert profile. |
{/* /HUMAN_AUTHORED:reconcile */}
