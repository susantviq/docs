...
description: "Orders to Email Attribution, a cross-channel view of how much SFCC order revenue your email channel actually drives. How to read it, why it matters, and how to act on it."
---
...
3. **SFCC is the denominator, and that is the point.** Trusting the ESP's own revenue total would hide the gap. By anchoring to SFCC's $4.2M, the card exposes the $1.62M that no channel is claiming. That is why this is a cross-channel card and not just an ESP report. Pair with [Total Revenue](/nerve-centre/kpi-cards/salesforce-commerce-cloud/total-revenue) to confirm the denominator.
...
This is a cross-channel card. It combines SFCC order revenue with email attribution from a connected ESP, so reconciliation means verifying **each side separately** and then confirming how the attribution join is built.
