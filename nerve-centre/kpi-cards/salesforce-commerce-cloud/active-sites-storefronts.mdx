...
| **Alert trigger** | none configured by default. Many merchants add an alert rule so any change in site count raises a flag, because a silent site drop is a high-impact, hard-to-spot event. |
...
Yes. There is no threshold configured by default because the right behaviour is merchant-specific, but most enterprise customers add an alert rule that flags any change in this count. A site silently going dark is high-impact and easy to miss, so an any-change alert is a common and sensible choice. Configure it in the Alert Rules tab for this card.
