...
The composition is set by the connector's risk model, but the sensitivity of each contributing signal (for example, what counts as an in-demand SKU for stock-out exposure, or how long an order can be unsynced before it counts) is configurable per profile in the Alert Rules tab. Tuning those changes what flows into this total.
