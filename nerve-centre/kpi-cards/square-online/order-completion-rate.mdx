...
**The Square unified-orders advantage:** because Square POS, Square Online, and Square Invoices all write to one Orders API with one `state` enum, this card gives a genuine cross-platform completion read that platforms with bolted-on POS cannot. Just remember the flip side: a single high headline can mask a struggling web channel, so split by `source.name` when web matters.
