---
title: "Net Revenue (after refunds + PayPal fees), PayPal"
sidebarTitle: "Net Revenue (after refunds + PayPal fees)"
description: "Gross transactions minus refunds minus PayPal fee_amount sums. How to read it, why it matters, and how to act on it."
---
...
> Gross transactions minus refunds minus PayPal fee_amount sums.

## At a glance

{/* HUMAN_AUTHORED:tldr */}
> **Net Revenue (after refunds + PayPal fees)** is a revenue & sales metric tracked from PayPal data. It surfaces operational signal at the revenue & sales layer so merchants can spot regressions, opportunities, and structural patterns in their store performance. The card pairs with sibling metrics in the Revenue & Sales category to build a complete diagnostic picture; cross-reference the related cards listed below for context.

| | |
| --- | --- |
| **What it counts** | Net Revenue (after refunds + PayPal fees) as exposed by the PayPal integration. The metric is computed from the latest available data and refreshed on the standard data refresh. |
...
| --- | --- |
| [`pp_avg_transaction`](/nerve-centre/kpi-cards/paypal/average-transaction-value) | Revenue & Sales sibling: Average Transaction Value. |
| [`pp_high_value_transactions`](/nerve-centre/kpi-cards/paypal/high-value-transactions) | Revenue & Sales sibling: High-Value Transactions. |
| [`pp_revenue_by_currency`](/nerve-centre/kpi-cards/paypal/revenue-by-currency) | Revenue & Sales sibling: Revenue by Currency. |
| [`pp_revenue_trend`](/nerve-centre/kpi-cards/paypal/revenue-over-time) | Revenue & Sales sibling: Revenue Over Time. |
...
{/* HUMAN_AUTHORED:faqs */}
**Q: How often does Net Revenue (after refunds + PayPal fees) update?**
...
**Q: How does Net Revenue (after refunds + PayPal fees) relate to other revenue & sales metrics?**
...
*Net Revenue (after refunds + PayPal fees)* is one of hundreds of KPI pulses Vortex IQ tracks across PayPal and 70+ other ecommerce connectors. Nerve Centre runs the detection layer; Vortex Mind investigates the cause when something moves; Ask Viq lets you interrogate any number in plain English.
