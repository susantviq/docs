---
title: "Net Revenue (after refunds + Stripe fees), Stripe"
sidebarTitle: "Net Revenue (after refunds + Stripe fees)"
description: "Gross charges minus refunds minus Stripe processing fees from balance_transactions. How to read it, why it matters, and how to act on it."
---
...
> Gross charges minus refunds minus Stripe processing fees from balance_transactions.

## At a glance

{/* HUMAN_AUTHORED:tldr */}
> **Net Revenue (after refunds + Stripe fees)** is a revenue & sales metric tracked from Stripe data. It surfaces operational signal at the revenue & sales layer so merchants can spot regressions, opportunities, and structural patterns in their store performance. The card pairs with sibling metrics in the Revenue & Sales category to build a complete diagnostic picture; cross-reference the related cards listed below for context.

| | |
| --- | --- |
| **What it counts** | Net Revenue (after refunds + Stripe fees) as exposed by the Stripe integration. The metric is computed from the latest available data and refreshed on the standard data refresh. |
...
| --- | --- |
| [`stripe_avg_transaction`](/nerve-centre/kpi-cards/stripe/average-transaction-value) | Revenue & Sales sibling: Average Transaction Value. |
| [`stripe_high_value_charges`](/nerve-centre/kpi-cards/stripe/high-value-charges) | Revenue & Sales sibling: High-Value Charges. |
| [`stripe_revenue_by_currency`](/nerve-centre/kpi-cards/stripe/revenue-by-currency) | Revenue & Sales sibling: Revenue by Currency. |
| [`stripe_revenue_trend`](/nerve-centre/kpi-cards/stripe/revenue-over-time) | Revenue & Sales sibling: Revenue Over Time. |
...
{/* HUMAN_AUTHORED:faqs */}
**Q: How often does Net Revenue (after refunds + Stripe fees) update?**
...
**Q: How does Net Revenue (after refunds + Stripe fees) relate to other revenue & sales metrics?**
...
*Net Revenue (after refunds + Stripe fees)* is one of hundreds of KPI pulses Vortex IQ tracks across Stripe and 70+ other ecommerce connectors. Nerve Centre runs the detection layer; Vortex Mind investigates the cause when something moves; Ask Viq lets you interrogate any number in plain English.
