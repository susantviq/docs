...
| **API resource & fields** | `GET /v1/charges` for live decline rate; commerce sibling (Shopify / BigCommerce / Adobe) for `checkout_attempts_per_min` and `aov`. Cross-platform derivation: this card cannot work without a commerce connector also being live. |
...
| [Decline vs Funnel](/nerve-centre/kpi-cards/stripe/decline-spike-vs-checkout-funnel-drop) | The companion cross-platform view. Confirms the spike is also showing up in the commerce sibling's checkout completion rate, separating "real lost revenue" from "buyer remorse". |
