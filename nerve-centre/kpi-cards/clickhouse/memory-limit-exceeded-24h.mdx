...
| **Alert Rules note** | This is an Alert Rules card: the `> 0` default is intentionally strict so the first occurrence surfaces. Teams with known-heavy ad-hoc workloads may raise the threshold per profile. |
...
3. **The alert threshold caught it at the first occurrence.** Because the threshold is `> 0`, the card went red at 16:02 on the very first kill, before the dashboard refreshed 13 more times. The team could have killed the saved tile or capped the user before the pattern repeated.
