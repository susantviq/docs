...
| **Sensitivity note** | This is an Alert Rules card: the `> 0` default is intentionally strict so the first occurrence surfaces. Teams with known-heavy ad-hoc workloads may raise the threshold per profile. |
