...
3. **Failed is categorically worse than slow.** A slow query eventually returns; a failed query returns an error to a human or a downstream job. A spike here means users and pipelines are seeing broken results right now, which is why it carries a Key Metrics, alert-bearing class.
{/* /HUMAN_AUTHORED:worked_example */}
...
100 is a generic default. If your baseline is single digits, lower the threshold in the Alert Rules tab to something like 40 so a doubling of your normal rate alerts. A high-volume cluster running millions of queries a day may legitimately see more than 100 trivial failures and should raise the threshold and rely more on [Query Error Rate %](/nerve-centre/kpi-cards/clickhouse/query-error-rate) instead.
