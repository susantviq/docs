...
No. Some merchants intentionally allow a loyalty code plus a shipping code. The card is named for the common case where stacking is unintended, but the threshold is configurable in the Alert Rules tab. Raise it if your store runs deliberate multi-coupon promotions, or lower it to zero-tolerance if you never combine codes.
