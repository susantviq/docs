...
| **Alert trigger** | `> 72h`. If the most recent successful backup is more than 72 hours old, the card pages the on-call DBA. Many teams tighten this in the Alert Rules tab to match their actual backup cadence (daily backups should alert well before 72 hours). |
...
72 hours is a deliberately loose floor so the card does not nag teams with weekly or infrequent backup policies. If you back up daily (or continuously via WAL), tighten the threshold in the Alert Rules tab to match: a daily-backup shop should probably alert at 26 to 30 hours so a single missed night is caught immediately rather than after three. The 72-hour default catches catastrophic neglect; your real RPO should drive the actual threshold.
