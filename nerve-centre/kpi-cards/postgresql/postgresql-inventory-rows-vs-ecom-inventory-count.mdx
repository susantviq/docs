...
Because one oversold hero product during a campaign is enough to cause real damage: cancelled orders, refunds, chargebacks, and reviews. A drift count of zero is the only safe state for an inventory pipeline, so any non-zero count is surfaced. Tune the sensitivity if your business tolerates a small standing offset (for example a reserved-stock buffer) in the Alert Rules tab.
