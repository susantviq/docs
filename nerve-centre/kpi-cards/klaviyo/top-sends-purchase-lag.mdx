...
| **Cross-platform attribution** | Klaviyo only sees its own touches. The join uses email address as the key, so a customer who received a Klaviyo email then later bought via a Google Ad on the same address shows up here as "email-attributed", overstating email's role. Same shape as `klv_total_revenue`. |
