...
| **API endpoint + report** | Amazon Ads API `GET /v2/sp/keywords/report` for spend per click, plus SP-API Pricing API + Listings Items API for the per-ASIN state, joined in our Vortex IQ Nerve Centre cross-platform index. |
...
This card is **inherently cross-platform**, joining Amazon Ads spend against SP-API ASIN state. Pair with these to act:
...
This card stitches both views in one number, which is why it's a cross-platform (`_xc_`) card and not native to either Amazon Ads or SP-API alone.
...
Because Shopify has no concept of Amazon Ads. The channel app pushes Shopify product data to Amazon catalogue listings; it does not interact with Amazon Sponsored Ads campaigns. The cross-platform join required for this card is between Amazon Ads and Amazon SP-API, both Amazon-internal. Shopify is irrelevant to this calculation.
