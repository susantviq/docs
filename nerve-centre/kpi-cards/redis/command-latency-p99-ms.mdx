...
Yes. If one `HGETALL` against a 500,000-field hash takes 80ms and it is called even a few times a minute, those calls land in the slowest 1% and set the p99. The instance can be otherwise idle. This is why p99 is a Key Metric card: it surfaces individually rare but individually severe events that averages hide completely.
