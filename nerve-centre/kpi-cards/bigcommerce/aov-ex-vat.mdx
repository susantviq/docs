...
**Q: Why does ex-VAT AOV not have its own alert threshold?**
Because the trend behaviour is identical to inc-VAT AOV when tax rates and customer mix are stable. The alert-threshold logic lives on `aov_trend`, which reads the percentage change. The ex-VAT version is a presentation variant for stores that prefer the net view.
