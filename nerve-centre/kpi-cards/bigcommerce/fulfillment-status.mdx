...
`Incomplete` is BC's "customer started but never finished checkout" state. It's typically excluded from the headline distribution because it represents abandoned carts, not orders. Pair with [BC Incomplete Rate](/nerve-centre/kpi-cards/bigcommerce/incomplete-cart-rate) for the abandoned-cart view.
