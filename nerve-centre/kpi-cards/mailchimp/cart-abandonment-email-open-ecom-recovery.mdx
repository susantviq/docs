...
| **Why cross-platform** | The first and last stages live in the store (abandonment, recovered order); the middle three live in Mailchimp (sent, open, click). Only by joining both can you see whether a recovery shortfall is an email problem or a store problem. |
...
| [`mailchimp-campaigns-ecom-revenue-90d`](/nerve-centre/kpi-cards/mailchimp/mailchimp-campaigns-ecom-revenue-90d) | The campaign-level cross-platform revenue view; complements this automation-level funnel. |
...
This is a cross-platform join, so no single dashboard holds the whole funnel. Reconciliation means confirming each stage against the system that owns it, and understanding the attribution join:
