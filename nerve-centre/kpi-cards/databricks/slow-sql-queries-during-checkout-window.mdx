...
| [Databricks SQL Spike vs Ecom Order Rate](/nerve-centre/kpi-cards/databricks/databricks-sql-spike-vs-ecom-order-rate) | The volume-side cross-channel peer. | Query spike with no order spike plus slow queries equals a runaway or rogue workload, not real shopper demand. |
| [Pipeline Lag vs Ecom Order Flow](/nerve-centre/kpi-cards/databricks/pipeline-lag-vs-ecom-order-flow) | The pipeline-side cross-channel peer. | Stalled pipeline plus slow serving queries can be the same root compute contention surfacing in two places. |
