...
**Metrics type:** [Key Metrics](/nerve-centre/overview#metrics-types-explained)  •  **Category:** [Pipelines](/nerve-centre/connectors#connectors-by-type)
...
| **Card class** | Key Metrics and Alert Rules card: it drives the Pipelines health signal and the expected-interval threshold is configurable in the Alert Rules tab. |
{/* /HUMAN_AUTHORED:tldr */}
...
| [Pipeline Lag vs Ecom Order Flow](/nerve-centre/kpi-cards/databricks/pipeline-lag-vs-ecom-order-flow) | The revenue cross-platform view. | Tells you whether the pipeline stalled while live orders were still flowing, the worst case. |
...
| **Expected-interval source** | Threshold differs | The alert uses the Alert Rules tab expected interval if set, which may not match the pipeline's literal schedule. |
...
It comes from the pipeline's schedule by default: a cron trigger, a continuous-pipeline cadence, or the interval you set in the Alert Rules tab. The alert fires when live lag exceeds that interval, so each pipeline is judged against its own cadence. If you reschedule a pipeline, update the expected interval so the threshold stays correct.
