...
- Cross-platform: JP ecom order has no Japan Post shipment after 48h (dispatch-SLA breach -> revenue at risk per channel)
- Cross-platform: actual_delivery > expected_delivery joined to commerce_sibling.order.total (late-delivery refund propensity)
- Cross-platform: EMS stuck AwaitingCustoms joined to sibling order -> proactive commercial-invoice / customs-doc fix candidates
- Cross-platform: ReturnedToSender by service_type joined to sibling orders -> return-cost attribution
