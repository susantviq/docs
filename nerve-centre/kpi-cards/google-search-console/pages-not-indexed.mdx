...
| **Alert trigger** | `> 10% of submitted URLs`, sentiment key for the indexing-coverage alert band. The 10% threshold is calibrated to ignore normal exclusions while catching a genuine de-indexing event. |
...
where `submitted_urls` is the count of canonical URLs you have declared through your sitemaps. The card crosses into its alert band when `not_indexed_share` exceeds **0.10 (10%)**.
