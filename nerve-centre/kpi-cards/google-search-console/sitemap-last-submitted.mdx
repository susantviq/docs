...
**Rule of thumb.** If you publish weekly or more often, treat anything over 7 days as worth a look and over 14 days as a definite investigation. If you publish rarely, a longer gap is expected and the alert can be relaxed in the Alert Rules tab.
{/* /HUMAN_AUTHORED:worked_example */}
...
No. Google reads sitemaps more often for sites that change often. A site that rarely publishes can go weeks between reads and be perfectly healthy. The 14-day default suits a site that publishes weekly. If you publish rarely, relax the threshold in the Alert Rules tab so the card does not cry wolf.
