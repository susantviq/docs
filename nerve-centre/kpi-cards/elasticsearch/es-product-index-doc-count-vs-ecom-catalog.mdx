...
| **Cross-platform join** | This is an Elasticsearch-distinctive cross-platform card: it only has meaning when an ecom connector is also configured, because the comparison number comes from the storefront, not from Elasticsearch. |
...
| [Search QPS Spike vs Ecom Traffic](/nerve-centre/kpi-cards/elasticsearch/search-qps-spike-vs-ecom-traffic) | Another cross-platform card joining search to storefront. | Read together to separate "search is incomplete" from "search is being hammered". |
| [ES Search Pool Saturation vs Ecom Burst](/nerve-centre/kpi-cards/elasticsearch/es-search-pool-saturation-vs-ecom-burst) | The capacity-side cross-platform peer. | Drift is a correctness problem; pool saturation is a capacity problem, distinct but often confused. |
{/* /HUMAN_AUTHORED:siblings */}
