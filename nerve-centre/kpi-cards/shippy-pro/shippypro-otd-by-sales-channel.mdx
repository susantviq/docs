...
| **Alert trigger** | `any channel <90%`. The card alerts as soon as any individual channel falls below 90% OTD even if the aggregate is healthy. This is the cross-platform point: you would never see an Amazon-channel issue from the headline number alone. |
...
The aggregate reads **94.6%** OTD, comfortably above 90%. The cross-platform cut surfaces that **Amazon IT alone is at 88.0%, alert FIRING**. Five things to notice:

1. **The aggregate hides the channel-level problem entirely.** Amazon at 88% is being absorbed by Shopify at 97% on three times the volume. Without the cross-platform cut, the operator sees "shipping is fine" while the Amazon Buyer-Promise scorecard at Amazon's end is dropping. This is exactly the cross-platform pattern this card exists to surface.
...
Cross-platform OTD answers "which channel is being under-served?" Pair with these to make the answer actionable:

| Card | Why pair it with OTD by Sales Channel | What the combination tells you |
| --- | --- | --- |
| [On-Time Delivery Rate](/nerve-centre/kpi-cards/shippy-pro/on-time-delivery-rate) | The aggregate counterpart. | Aggregate green + a channel red = the cross-platform cut is the only place the issue is visible. Aggregate red + all channels red = network-wide degradation. |
...
| [OTD by Route](/nerve-centre/kpi-cards/shippy-pro/otd-by-route) | Per-carrier OTD. | Cross-platform underperformance + a single carrier underperformance = the channel is over-routed to that carrier. |
