...
| **What counts as a failure** | Vortex IQ may read higher | We count handler-returned 4xx as failures by default; the Supabase invocation chart may class only 5xx and runtime errors as errors. Adjust the failure definition in Alert Rules if you only care about 5xx. |
