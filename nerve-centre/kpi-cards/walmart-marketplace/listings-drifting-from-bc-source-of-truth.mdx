...
| [`wal_xc_listed_but_oos_on_bc`](/nerve-centre/kpi-cards/walmart-marketplace/active-walmart-listings-for-oos-skus-on-bc) | Another cross-channel mismatch, this time on availability. |
...
Seller Center shows only the Walmart side and has no view of your BigCommerce values, so it cannot show drift on its own. This card is a cross-channel comparison, which is why the count only exists in Vortex IQ.
