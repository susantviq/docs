...
MySQL-specific health audit for production 5.7 / 8.0 instances. Answers six questions: (1) is access correctly scoped and is performance_schema actually readable; (2) is the instance reachable with a healthy connection pool and low aborted-connect noise; (3) is query latency within band and is the slow-query digest list under control; (4) are replicas streaming with both threads running and Seconds_Behind_Source inside threshold; (5) is disk and buffer-pool capacity healthy; (6) is a recent durable backup in place. Cross-platform area joins query and pool pressure to commerce-sibling checkout traffic to size live revenue at risk.
