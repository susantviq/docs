...
The Dashboard Settings page (gear icon, Alert Rules tab) is the surface for sentiment customisation. The screenshots show the layout: a per-metric row with a green target value, a red threshold value, a sensitivity slider (very high / high / medium / low), and a "Reset to Defaults" button per category.
