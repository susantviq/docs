...
Cross-channel cards (ad spend on out of stock products, marketplace drift, email share of revenue) light up once an Odoo store is connected alongside Google Ads, Amazon, or Klaviyo.

## Detailed KPI cards in active development

The Odoo adapter is in build. Each card will document the exact Odoo model and field used (sale.order, product.product, account.move, stock.quant, res.partner), the calculation, why it matters, and the playbook for when sentiment shifts. Cards follow the same hero / non-hero / cross-channel layout as the rest of the Nerve Centre, so a finance lead and a marketing lead share one screen.
