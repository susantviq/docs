...
3. **The card fired the alert at roughly 09:30,** once the morning cluster crossed `>5`. That is hours before the merchant would have noticed from a quiet sales total, and before the lunchtime peak made it worse. Speed is the whole point of a 24-hour window on a Key Metrics card.
