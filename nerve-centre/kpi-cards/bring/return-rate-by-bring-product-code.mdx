...
| **Cross-platform join** | Returns are counted regardless of which sales surface the order came from; the split is by Bring product code, not by channel. A marketplace order and a D2C order shipped on the same code land in the same bucket. |
