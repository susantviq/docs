...
| **Cross-platform join** | Orders arrive from every sales surface (own store, marketplaces, wholesale). This card attributes the dispatch miss back to the order, so a marketplace order and a D2C order are scored on the same despatch clock. |
