...
2. **The Spark Ads boost fell harder (-72%) but does not fire.** It is below the configured spend floor, so a one-order swing is expected. Raising or lowering that floor is an Alert Rules-tab decision based on how much daily spend counts as material for this account.
