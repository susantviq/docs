  ...
  <Tab title="Monitor (20)">
    ...
    <AccordionGroup>
      <Accordion title="Run operations (10 signals)">
        ...
        | --- | --- | --- |
        | **Alerts Acknowledged** | Merchant rule | Incidents moved to identified - cause is known and a fix is in flight. |
        | **Alerts Firing** | Alert band 0 / 3 | Published incidents still in the investigating state - the page is openly telling customers something is wrong. |
        | **Incidents Open** | Alert band 0 / 3 | Unresolved incidents published on the status page right now. |
        | **Incidents Resolved (24h)** | Merchant rule | Incidents closed in the last 24 hours - recovery throughput. |
        | **Mean Time To Acknowledge** | Alert band 300000 / 900000 | Average gap from incident creation to the identified/monitoring update - how fast the team confirms a cause. |
        | **Mean Time To Resolve** | Alert band 1800000 / 3600000 | Average gap from incident creation to resolution - how fast published incidents actually get closed. |
        | **SLA Compliance** | Alert band 99.9 / 99.5 | Rolling component-group availability vs the SLA target. 99.9% is two-nines-plus; below 99.5% is a breach. |
        | **Services Down** | Alert band 0 / 1 | Components in major_outage - customer-facing outage surface published to the status page. |
        | **Services Healthy** | Alert band 0 / -10 | Components reporting operational across the status page. |
        | **Top Alerting Services** | Merchant rule | Components / groups generating the most incidents - where to spend reliability effort first. |
      ...
      <Accordion title="Customer experience (7 signals)">
        ...
        | Signal | Alert behaviour | What it tracks |
        | --- | --- | --- |
        | **Apdex Score** | Alert band 0.95 / 0.7 | Satisfied/tolerating/frustrated ratio derived from the system metrics published to the status page. 0.94+ is healthy. |
        ...
        | **Services Degraded** | Alert band 0 / 2 | Components in degraded_performance or partial_outage - the early-warning band before a full outage. |
      ...
      <Accordion title="Protect revenue (2 signals)">
        ...
        | Signal | Alert behaviour | What it tracks |
        | --- | --- | --- |
        | **Checkout Component Health × Sales** | Merchant rule | Checkout-component status overlaid with order volume - when the component degrades, sales follow. |
        | **Revenue at Risk (live)** | Merchant rule | Live $/min loss while any component is in major_outage and a commerce sibling reports velocity. The COO's number. |
      ...
      <Accordion title="Control risk and change (1 signals)">
        ...
        | Signal | Alert behaviour | What it tracks |
        | --- | --- | --- |
        | **Top Error Types** | Merchant rule | Most common incident impact bands / failure causes across published incidents. |
