...
[Connect or manage this source](https://app.vortexiq.ai/workbench/settings/sources) &middot; [How connecting works](/integrations/connector-catalogue) &middot; [Create a workflow](https://app.vortexiq.ai/workbench/flows/create?connector=uptime-robot)
  ...
  <Tab title="Monitor (20)">
    ...
    <AccordionGroup>
      <Accordion title="Run operations (11 signals)">
        ...
        | **Alerts Firing** | Merchant rule | Alert events raised by Uptime Robot monitors in the window. |
        | **Incidents Open** | Alert band 0 / 3 | Outage windows currently open (down or degraded) across all monitors. |
        | **Incidents Resolved (24h)** | Merchant rule | Outage windows closed in the last 24 hours - recovery throughput. |
        | **Mean Time To Acknowledge** | Alert band 300000 / 900000 | Median gap between a monitor going down and a responder acknowledging it. |
        | **Mean Time To Resolve** | Alert band 1800000 / 3600000 | Median gap between a monitor going down and recovering to up. |
        | **SLA Compliance** | Alert band 99.9 / 99.5 | Uptime ratio measured against the SLA target across the window. |
        | **Services Degraded** | Alert band 0 / 2 | Monitors responding slowly or intermittently (seems_down) but not fully down. |
        | **Services Down** | Alert band 0 / 1 | Monitors fully unreachable (down). The single most-watched number for an SMB operator. |
        | **Services Healthy** | Alert band 1 / 0 | Monitors currently reporting an up / healthy state. |
        | **Top Alerting Services** | Merchant rule | Monitors that raised the most alerts in the window - the noisiest / least-stable services. |
      ...
      <Accordion title="Customer experience (6 signals)">
        ...
        | **Avg Response Time** | Alert band 200 / 1500 | Average monitor response time across all checks. |
        | **Error Rate** | Alert band 0.5 / 2 | Share of checks returning an error / down state. |
        | **Throughput (req/min)** | Alert band 0 / -10 | Checks executed per minute across all monitors. |
        | **p95 Latency** | Alert band 800 / 1500 | 95th-percentile monitor response time - the slow-tail experience. |
        | **p99 Latency** | Alert band 1500 / 3000 | 99th-percentile monitor response time - worst-case slow checks. |
      ...
      <Accordion title="Protect revenue (2 signals)">
        ...
        | Signal | Alert behaviour | What it tracks |
        | --- | --- | --- |
        | **Revenue at Risk (live)** | Merchant rule | When a storefront monitor is down, price the outage in $/min using the commerce sibling's revenue baseline. The COO's number. |
        | **Storefront Down During Campaign** | Merchant rule | Paid traffic landing on a down storefront. Overlay outage windows with active ad-campaign spend to surface money burned on traffic that cannot convert |
      ...
      <Accordion title="Control risk and change (1 signals)">
        ...
        | Signal | Alert behaviour | What it tracks |
        | --- | --- | --- |
        | **Top Error Types** | Merchant rule | Most common down reasons (timeout, connection refused, status code, keyword-not-found, DNS) across failing monitors. |
