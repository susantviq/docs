...
<Tabs>
  ...
  <Tab title="Audit (12)">
    ...
    <AccordionGroup>
      ...
      <Accordion title="WLM Queue Length &gt;5 Sustained 5m">

        **Severity** high &middot; **Outcome** Run operations &middot; **Fix status** Report only

        Queues sustained above 5 in workload management mean the cluster is undersized or a workload is misplaced; everything behind it runs late.

        Vortex IQ detects and explains this; resolution is manual, with evidence and recommended steps.

        Reference: `RS-CAP-003`
      ...
      <Accordion title="Queries Spilling to Disk &gt;10 in 24h">

        **Severity** medium &middot; **Outcome** Customer experience &middot; **Fix status** Report only

        Queries spilling to disk run orders of magnitude slower than in memory; the offending queries are identifiable and usually tunable.

        Vortex IQ detects and explains this; resolution is manual, with evidence and recommended steps.

        Reference: `RS-PERF-004`

      </Accordion>
      <Accordion title="Tables With &gt;10% Unsorted Rows (VACUUM Candidate)">

        **Severity** medium &middot; **Outcome** Customer experience &middot; **Fix status** Report only

        Tables over 10 percent unsorted degrade every scan that touches them; VACUUM restores the performance already paid for.

        Vortex IQ detects and explains this; resolution is manual, with evidence and recommended steps.

        Reference: `RS-MAINT-001`

      </Accordion>
      <Accordion title="Tables With Stale Statistics (ANALYZE Candidate)">

        **Severity** low &middot; **Outcome** Customer experience &middot; **Fix status** Report only

        Stale statistics mislead the query planner into bad plans; ANALYZE is the cheapest performance fix the cluster has.

        Vortex IQ detects and explains this; resolution is manual, with evidence and recommended steps.

        Reference: `RS-MAINT-002`
