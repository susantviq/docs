...
Using **Selective Content Migration**, you can migrate a single Product or few products to the destination environment (store or storefront).

---

## Feature Notes

- **Schedule Copy** - Migration scheduling will be available in an upcoming release as part of the upgraded migration API. Please contact support if you need early access.
- **Copy Products** allows you to update only selected checkbox items from source to destination.
...
This mode is significantly faster than full migration because no components are recreated. There is no backup or rollback because the operation is non-destructive: removing the channel assignment via BigCommerce admin reverses it.

Common use cases for same-store channel migration: launching a B2B channel from a B2C catalogue, regional expansion (US storefront to UK storefront), brand separation, outlet store, partner portal.
