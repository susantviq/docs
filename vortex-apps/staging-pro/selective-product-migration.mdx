...
- Brand: included by default (only referenced brands migrate); untick it, with a confirmation, to migrate the products without their brand link
- Categories: included by default (only referenced categories and their parents migrate); untick it, with a confirmation, to migrate the products without their category links
...
The options popup also offers **Hide products on the destination** (off by default, reset each time the popup opens). Every product the run creates or updates is saved as hidden on the destination, including products already live there, so you can migrate to a production store ahead of a launch, review, and then make the products visible on the destination. The products table's **Visible** column shows each product's current state on the source, which is the pre-flight check for the same workflow. The choice is recorded on the run's History row and detail as *Product visibility on destination: hidden*.

## Brands and categories handling

When Brand and Categories are included, which is the default:
...
When you exclude Brand or Categories in the options popup, none of this happens for that entity: the product lands (or is updated) without brand or category links, a destination product keeps whatever links it already has, and no brand or category is created or deleted. Use it when the destination keeps its own brand list or category tree.
