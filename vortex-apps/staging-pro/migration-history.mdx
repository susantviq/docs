---
title: "Migration History in StagingPro for BigCommerce"
sidebarTitle: "Migration History"
description: "The Migration History tab (runs, scheduled rows, run detail, issue log) and the Code Deployments tab (approval, logs, theme rollback)."
---
...
- **Choices**: run options recorded when the run was started, for example *Product visibility on destination: hidden* when [Hide products on the destination](/vortex-apps/staging-pro/selective-content-migration#products) was ticked. The entity grid above also carries a *Hide products on the destination* cell for every product run, stating either *Yes* or *No, kept unselected*, so an unticked option is stated rather than implied.
