...
<Note>
  Always send `Accept: application/json`. Without it, authentication failures can return a login
  redirect or an HTML error page instead of the JSON errors documented here.
</Note>
...
- Tokens live for **10 days** by default (`expires_in` in the login response is authoritative). Log
  in again to get a fresh one - there is no refresh endpoint.
...
| `429` | Login rate limit exceeded (10 per minute), or a run was paused more than 3 times within an hour (`restart_loop_protection` — see Pause migration). |
