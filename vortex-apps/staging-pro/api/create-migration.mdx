...
<AccordionGroup>
  ...
  <Accordion title="Hide products on the destination" icon="eye-slash">
    `hide_products_on_destination: true` lands the migrated products on the destination as hidden
    (not visible on the storefront) so they can be reviewed before going live. It is accepted in two
    shapes, matching the app:

    - a **selective product run** (`selection.products`, `conflict_strategy: "update"`): every
      product the run creates **or updates** is saved hidden, including products already live on the
      destination. Re-enable them on the destination after review.
    - a **bulk run with `conflict_strategy: "skip"`** (Add New Only): only the products the run
    ...
    the source visibility) and under a bulk `update` (it would hide every matched product on a live
    store), and with `422 true_replica_fixed_configuration` in `mode: "true-replica"`. The flag is
  ...
  <Accordion title="Two heavy migrations never share a store" icon="traffic-cone">
    A **bulk** create (any mode, including dry-run and verify) and a **true-replica** create are
    heavy; a **selective** create is light. Two migrations share a store when either one's source
