...
<Warning>
  **Selecting a parent does not include its children, each child is opt-in.** `selected_entities:
  ...
  for API-token callers: `custom_fields` are automatically included on a **selective** product run
  (a compatibility behaviour of this API - the UI sends an explicit choice).
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Content, pages, theme, email, blog, banners, scripts" icon="file-text">
    ...
    <Note>
      `webdav_files` copies files using both stores' WebDAV credentials, but only the **source**
      credentials are enforced up front: a run that selects it without them is refused with
      `422 source_webdav_required` rather than silently skipped - the sync reads the files straight
      from the source store's file storage. Destination credentials are used when on file. The
      engine only ever reads from the source (never writes to it).
    </Note>
  ...
  <Accordion title="B2B Edition" icon="building">
    Both stores must be on B2B Edition, with store API tokens created with the **B2B Edition** scope.

    | Key | Migrates | Notes |
    | --- | --- | --- |
    | `b2b_roles` | Company roles & permissions | No personal data — no anonymisation needed. |
    | `b2b_companies` | Companies (status, payment configuration, hierarchy, price-list links) | **PII — requires `anonymise_customer_data: true`.** Select `customer_groups` and `price_lists` in the same run or companies are created without their pricing link. |
    | `b2b_addresses` | Company addresses | **PII — requires anonymisation.** |
    | `b2b_users` | Company users (buyers) | **PII — requires anonymisation.** |
    | `b2b_super_admins` | Super admins and their company assignments | **PII — requires anonymisation.** |
    | `b2b_sales_staff` | Sales-rep-to-company assignments | The rep's account must already exist on the destination (accounts are not API-creatable). |
  </Accordion>
...
**Brands and categories ride with products only when you list them.** With `brands` and
`categories` in `selected_entities` alongside `products`, a selective product run migrates only the
brands and categories the picked products reference (plus category **ancestors**, so the tree is
intact). Leave either key out to migrate the products **without** those links: no brand or category
is created or deleted, new products land without the link, and existing destination products keep
the brand or categories they already have (the app's **Include Brand** / **Include Categories** ticks
send exactly this). For API-token callers, a product's custom fields also travel automatically on a
selective product run, even if `custom_fields` isn't listed.
...
<AccordionGroup>
  <Accordion title="Customer data must be anonymised" icon="user-lock">
    Selecting `orders`, `customers`, `gift_certificates`, or any of the B2B personal-data entities
    (`b2b_companies`, `b2b_addresses`, `b2b_users`, `b2b_super_admins`) requires
    `anonymise_customer_data: true`. Without it the request is rejected:
  ...
  <Accordion title="Source and destination must differ" icon="ban">
    ...
    production destination additionally refuses `conflict_strategy: "replace"` (see the strategy
    accordion above) — other strategies are allowed against it.
  </Accordion>
  ...
  <Accordion title="True Replica: the platform owns the configuration" icon="copy">
    ...
    - True Replica is on for every organisation by default; if it has been explicitly disabled for
      yours, requests return `403 true_replica_not_enabled`.
  ...
  <Accordion title="Hide products on the destination" icon="eye-off">
    `hide_products_on_destination: true` lands the migrated products on the destination as hidden
    (not visible on the storefront) so they can be reviewed before going live. What lands hidden
    depends on the strategy:

    - with `conflict_strategy: "update"` on a **selective product run** (`selection.products`,
      which is what the app's selective runs send): every product the run creates **or updates** is
      saved hidden, including products already live on the destination. Re-enable them on the
      destination after review.
    - with `conflict_strategy: "skip"` (Add New Only, bulk or selective): only the products the run
      creates land hidden; matched products are never written.

    Refused with `422 hide_products_not_allowed_with_strategy` under `replace` (an exact copy keeps
    the source visibility) and under `update` when no products are picked (a bulk update would hide
    every matched product on a live store), and with `422 true_replica_fixed_configuration` in
    `mode: "true-replica"`. The flag is
    only honoured once the migration service confirms it supports it: `422 hide_products_support_unknown`
    means the check could not be made right now (retry in a moment), `422 hide_products_not_available`
    means the deployed service does not offer it yet. The choice is recorded on the run and comes
    back as `runOptions.hide_products_on_destination` on
    [List migrations](/vortex-apps/staging-pro/api/list-migrations) and
    [Get migration](/vortex-apps/staging-pro/api/get-migration).
  </Accordion>
  <Accordion title="Two heavy migrations never share a store" icon="traffic-cone">
    A **bulk** create (any mode, including dry-run and verify) and a **True Replica** create
    (`mode: "true-replica"`) are heavy; a **selective** create is light. Two migrations share a store when either one's source
    or destination is the other's source or destination, in any direction. A heavy create is
    refused:

    - with `422 stores_busy` while a heavy run is active (`queued`, `in_progress` or `paused`)
      on a shared store; the message names the running migration.
    - with `422 schedule_imminent` while a bulk or True Replica migration
      [scheduled from the app](/vortex-apps/staging-pro/scheduled-migrations) on a shared store is
      due within the next 2 hours; the message names the schedule.

    Selective creates are exempt both ways: they are never refused for a busy store, and they
    never make a heavy create wait. Retry a refused create once the running migration has
    finished or the scheduled one has started.
  </Accordion>
</AccordionGroup>
...
| `selected_entities`, `selection`, `theme_activation`, `conflict_strategy`, or `hide_products_on_destination` alongside `mode: "true-replica"` | `422 true_replica_fixed_configuration`. |
...
| A bulk or True Replica create while a bulk or True Replica run is active on a shared store | `422 stores_busy`, naming the running migration. |
| A bulk or True Replica create while a scheduled bulk or True Replica migration on a shared store is due within 2 hours | `422 schedule_imminent`, naming the schedule. |
| `hide_products_on_destination: true` with `replace`, or with `update` on a bulk run | `422 hide_products_not_allowed_with_strategy`. |
| `hide_products_on_destination: true` while the migration service cannot confirm it supports the option, or does not offer it | `422 hide_products_support_unknown` (retry shortly) or `422 hide_products_not_available`. |
