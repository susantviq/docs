...
<Note>
  **Scheduling is in-app only.** The API has no schedules endpoint; migrations are scheduled for a
  later date and time from the StagingPro UI (see
  [Scheduled migrations](/vortex-apps/staging-pro/scheduled-migrations)). Once a scheduled run has
  started it appears in [List migrations](/vortex-apps/staging-pro/api/list-migrations) like any
  other run, with a run id that starts with `SR-`.
</Note>
