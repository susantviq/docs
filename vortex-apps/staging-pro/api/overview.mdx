...
Sizing a migration, launching it, controlling it, and streaming its progress - everything the
[StagingPro](/vortex-apps/staging-pro/index) BC Migration app does with a run - is available over
REST, so you can drive migrations from your own tooling instead of the UI. (The in-app store
*browsing* screens are UI-only and not part of this API.)
...
<Steps>
  ...
  <Step title="Follow it">
    Subscribe to `GET /migrations/{id}/stream` for live Server-Sent Events — with a bearer token
    that is also the only live view, since `GET /migrations/{id}` and `/issues` serve finished runs
    (they return `403 not_available` to token callers while a run is still live). Once the run
    finishes, `GET /migrations/{id}/issues` itemises anything that failed, was intentionally
    skipped, or was fixed automatically by the self-healing layer, with reasons.
  </Step>
  <Step title="Verify">
...
| `true-replica` | A server-configured exact copy: the platform picks the entities itself (everything storefront-visible, no PII), then runs an automated verification pipeline (heal, compare, delta, report) that crawls both storefronts and produces a store-owner report. Do not send `selected_entities`, `selection`, `theme_activation`, or `conflict_strategy` in this mode. Requires destination WebDAV credentials; available unless explicitly disabled for your organisation. |

<Note>
  **Scheduling is in-app only.** Migrations are scheduled for a later date and time from the
  StagingPro UI (see [Scheduled migrations](/vortex-apps/staging-pro/scheduled-migrations)). The
  schedule endpoints under `/schedules` accept the signed-in app session only: a call that carries a
  bearer token is refused with `403 not_available` ("Scheduling is not available with API token
  authentication"). Once a scheduled run has started it appears in
  [List migrations](/vortex-apps/staging-pro/api/list-migrations) like any other run, with a run id
  that starts with `SR-`.
</Note>
...
- **Customer data must be anonymised.** Selecting `orders`, `customers`, `gift_certificates`, or
  the B2B personal-data entities requires `anonymise_customer_data: true`.
...
- **Two heavy migrations never share a store.** A bulk or true-replica create is refused with
  `422 stores_busy` while a bulk or true-replica run is active on a store it shares (as source or
  destination, in either direction), and with `422 schedule_imminent` while a scheduled bulk or
  true-replica migration on a shared store is due within the next 2 hours. Selective runs are
  exempt both ways.
