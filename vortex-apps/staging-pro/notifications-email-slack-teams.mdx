...
When a migration is [scheduled for later](/vortex-apps/staging-pro/scheduled-migrations), the person who scheduled it receives an email when it starts, if it could not start, or if it missed its slot (the StagingPro helpdesk is copied on a miss). These emails go to that person's account whatever is configured on this page, for as long as they are a member of your organisation. Once the migration has started it is an ordinary migration, so the **Data Migration Notifications** above apply to it as well. The emails and what triggers each are described under [The emails](/vortex-apps/staging-pro/scheduled-migrations#the-emails) on the Scheduled migrations page.
