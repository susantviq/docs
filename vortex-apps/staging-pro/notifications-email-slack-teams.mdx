...
## Scheduled migration emails

When a migration is [scheduled for later](/vortex-apps/staging-pro/scheduled-migrations), the account that scheduled it receives an email at the outcome, whatever is configured on this page:

| Email | When |
| --- | --- |
| **Your scheduled migration has started** | The migration started at its time, or after waiting for another migration on the same store to finish |
| **Your scheduled migration could not start** | A start-time check failed. The email carries the reason and what to do |
| **A scheduled migration missed its slot** | It could not be started within 15 minutes of its time, or the stores were still busy 2 hours after it. The StagingPro helpdesk is copied so the cause can be checked |

Each email names the stores, the type of run, the entities, the scheduled time and the requester, and links to Migration History. Once a scheduled migration has started it is an ordinary migration, so the Data Migration Notifications above apply to it as well.

<Frame>
  ![The email sent when a scheduled migration missed its slot](/images/staging-pro/scheduling/schedule-email-missed.png)
</Frame>
