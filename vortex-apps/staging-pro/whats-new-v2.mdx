...
| **Image files** | Destination pulls images from the source store's public URLs | **Direct file transfer (WebDAV)**; Page Builder widget images too (opt-in, needs destination WebDAV credentials) |
...
| **Scheduling a migration** | Calendar scheduling, Premium tier only | **Schedule for later** on every migration: a date, a time and a timezone, protection against two heavy migrations sharing a store, and an email on the outcome |
...
- **Pause** a running migration whenever you like, before a traffic peak, for example, and resume it later. It carries on exactly where it stopped. (A guard refuses a fourth pause of the same run within an hour — a quiet progress bar usually means the destination is digesting, not a hang.)
...
<Warning>
  ...
  - A store marked **Production** is guarded as a *destination*: the confirmation dialog shows a red PRODUCTION warning and asks you to confirm you understand, and **Make Exact Copy** is refused against it (it deletes data).
...
### 14. Schedule it for later

Any migration you could start now can be scheduled instead. Pick a date, a time and a timezone, and StagingPro starts it then with exactly the options you chose. Two large migrations (bulk or True Replica) are never started on the same store at the same time: a scheduled run waits for a running one to finish, and a manual start is refused while a scheduled run on the same store is due within the next 2 hours. You get an email when the migration starts, if it could not start, or if it was missed. See [Scheduled migrations](/vortex-apps/staging-pro/scheduled-migrations).
...
<Note>
  **One capability stays on the classic app for now**, the backup & restore subsystem. If you rely on it, keep using classic for that task; it is unchanged. Everything else, including B2B Edition data migration and [scheduled migrations](/vortex-apps/staging-pro/scheduled-migrations), is covered in V2.
</Note>
