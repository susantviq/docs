...
Every connector at parity contributes its native entities and metrics. Products, orders, customers, channels, inventory, campaigns, ad creatives, email sends, support tickets, parcel events, payment transactions, incident records, all live, all current. This is the layer the [Nerve Centre](/nerve-centre/overview) reads to compute its KPI cards and the layer the [Cross-Platform Metrics Kill-Shots](/integrations/connector-catalogue) reach across to compute joined values.

Currently 41 connectors contribute live nodes and edges, with 148 cross-platform cards already joining two or more connectors at the data layer. The full per-connector inventory is in the [Cross-Platform Metrics Kill-Shots](/integrations/connector-catalogue) catalogue.
...
- Connector graph for the 41 connectors at parity. The cross-platform join layer is live and the [148 Cross-Platform Metrics Kill-Shot cards](/integrations/connector-catalogue) read from it.
...
- 41 connectors at Key-Metrics-plus-Manifest-plus-Graph parity, contributing live nodes and edges per tenant
- 148 Cross-Platform Metrics cards joining two or more connectors at the data layer
- Over 7,000 unique KPI cards in the compiled registry
...
No. The connector graph (Layer 1) lights up the moment you connect two or more connectors. The cross-platform join cards documented in the [Cross-Platform Metrics Kill-Shots](/integrations/connector-catalogue) catalogue work without the change-history layer.
...
- The full per-connector index of cross-platform cards: [Cross-Platform Metrics Kill-Shots](/integrations/connector-catalogue)
- The metrics types that govern how the graph's cards are surfaced: [Key Metrics, Cross-Platform Metrics and Supporting Metrics types](/integrations/connector-catalogue)
